...
class Worker(AggregateRoot):
    ...
    profile: CapabilityProfile = Field(
        default_factory=CapabilityProfile, description="Capabilities & Skills"
    )
    reputation: ReputationScore = Field(
        default_factory=ReputationScore, description="Performance metrics"
    )
